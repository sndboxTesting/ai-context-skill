...
export PATH="$HOME/SuneelWorkSpace/bin:$PATH"
echo 'export PATH="$HOME/SuneelWorkSpace/bin:$PATH"' >> ~/.zshrc
```

**Validate:** `which adwi` → should show `~/SuneelWorkSpace/bin/adwi`
...
cp ~/SuneelWorkSpace/config/.env.example ~/SuneelWorkSpace/config/.env

# Edit and fill in real values
$EDITOR ~/SuneelWorkSpace/config/.env
```
...
```bash
cd ~/SuneelWorkSpace/mcp-servers/obsidian-bridge
# Start it (stdlib-only — no pip install needed)
python3 server.py &
# Or use the bin script:
~/SuneelWorkSpace/bin/start-obsidian-bridge
```
...
```bash
~/SuneelWorkSpace/bin/start-command-api
```
...
python3 ~/SuneelWorkSpace/scripts/validate_adwi_env.py
