...
REPORT_RESOURCES = {
    "workspace://audit/system": "audit/system_audit.md",
    "workspace://audit/gaps": "audit/gap_analysis.md",
    "workspace://audit/improvement-plan": "audit/improvement_plan.md",
    ...
    "workspace://research/index": "research-engine/ideas/index.json",
    "workspace://research/decisions": "research-engine/decisions/README.md",
    "workspace://memory/patterns": "agent-system/memory/PATTERNS.md",
    "workspace://memory/insights": "agent-system/memory/INSIGHTS.md",
    "workspace://policy/bounded-self-upgrade": "agent-system/shared/BOUNDED_SELF_UPGRADE.md",
    "workspace://orchestrator/system-intelligence-policy": "orchestrator/router/system_intelligence_policy.md",
    "workspace://goals/idea-pipeline": "goal-engine/planner/idea_execution_pipeline.md",
    "workspace://autolab/system-improvement-strategy": "autolab/meta/system_improvement_strategy.md",
}
...
def gap_items() -> list[dict[str, str]]:
    ...
    return [
        ...
        {
            "category": "automation",
            "gap": "Health checks did not summarize audit/gap/research/tool readiness.",
        ...
        },
        {
            ...
            "fix": "Register new resources in mcp/server/config/resource_map.json.",
            "priority": "P1",
        },
        {
            "category": "automation",
...
def update_mcp_resource_map() -> None:
    path = ROOT / "mcp/server/config/resource_map.json"
    data = read_json(path, {})
...
def update_health() -> dict[str, Any]:
    health_path = ROOT / "agent-system/state/WORKSPACE_HEALTH.json"
    health = read_json(health_path, {})
    checks = {
        "system_audit": (ROOT / "audit/system_audit.md").exists(),
        "gap_analysis": (ROOT / "audit/gap_analysis.md").exists(),
        "improvement_plan": (ROOT / "audit/improvement_plan.md").exists(),
        ...
        "mcp_resource_coverage": all(
            str(v) in json.dumps(read_json(ROOT / "mcp/server/config/resource_map.json", {}))
...
def status_text() -> str:
    health = read_json(ROOT / "agent-system/state/WORKSPACE_HEALTH.json", {})
...
def main() -> int:
    ...
    if args.command == "capabilities":
        ...
        if args.command == "audit":
            print(ROOT / "audit/system_audit.md")
        elif args.command == "gaps":
            print(ROOT / "audit/gap_analysis.md")
