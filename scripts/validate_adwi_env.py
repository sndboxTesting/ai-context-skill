...
def chk_env_file() -> tuple[str, str]:
    ...
    if not env.exists():
        return "warn", "config/.env not found — copy config/.env.example → config/.env and fill in API keys"
    keys = {}
...
def chk_simeval_artifacts() -> tuple[str, str]:
    simeval = WORKSPACE / "logs" / "simeval"
    report = simeval / "MASTER_REPORT_v2.md"
    backlog = simeval / "fix_backlog_v2.json"
    if not simeval.exists():
        return "warn", "logs/simeval/ not found — eval artifacts not present"
    missing = []
    ...
    return "pass", f"Eval artifacts present in logs/simeval/"


def chk_bin_executable() -> tuple[str, str]:
    adwi_bin = WORKSPACE / "bin" / "adwi"
    if not adwi_bin.exists():
        return "fail", "bin/adwi not found"
    if not os.access(adwi_bin, os.X_OK):
        return "warn", "bin/adwi exists but not executable — run: chmod +x bin/adwi"
    in_path = subprocess.run(["which", "adwi"], capture_output=True).returncode == 0
    if not in_path:
        return "warn", "bin/adwi not in $PATH — add ~/SuneelWorkSpace/bin to PATH in ~/.zshrc"
    return "pass", "bin/adwi executable and in PATH"
